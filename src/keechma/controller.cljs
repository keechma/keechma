...
(defmulti params record-type)
(defmulti start record-type)
(defmulti wake record-type)
(defmulti stop record-type)
(defmulti handler record-type)
(defmulti ssr-handler record-type)
(defmulti context record-type)
(defmulti report record-type)
(defmulti execute record-type)
(defmulti send-command record-type)
(defmulti is-running? record-type)
(defmulti redirect record-type)
...
(defmethod report :default
  ([controller direction name payload] (report controller direction name payload :info))
  ([controller direction name payload severity]
     (let [reporter (or (:reporter controller) (fn [_ _ _ _ _ _ _]))
...
(defmethod execute :default
  ...
  ([controller command-name args]
   (report controller :in command-name args)
   (put! (:in-chan controller) [command-name args])))
(defmethod send-command :default
  ([controller command-name]
   (send-command controller command-name nil))
  ([controller command-name args]
   (let [out-chan (:out-chan controller)]
     (report controller :out command-name args)
     (put! out-chan [command-name args])
     controller)))
